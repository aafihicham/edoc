...
export default {
  data() {
    return {
      ...
      email: '',
      password: '',
      confirmPassword: '', // Added confirmPassword to store user input
    };
  },
  methods: {
    async submitForm() {
      try {
        // Include confirmPassword in the request payload
        const response = await http.post('http://127.0.0.1:8000/api/register', {
          name: this.name,
          email: this.email,
          password: this.password,
          password_confirmation: this.confirmPassword, // Include this field for confirmation
        });
